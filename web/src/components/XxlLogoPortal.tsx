...
export interface XxlLogoPortalProps {
  variant?:     Variant;
  lang?:        Lang;
  forceStatic?: boolean;
  className?:   string;
...
// Separate session key so portal animation plays independently of super.xxl.co.il
const SESSION_KEY = 'xxl_portal_animated_this_session';
...
export default function XxlLogoPortal({
  variant     = 'hero',
  lang        = 'he',
  forceStatic = false,
  className,
  ...
  const arcId = `xxl-portal-arc-${uid}`;
  ...
  if (variant === 'favicon') {
    return (
      <svg viewBox="0 0 320 200" xmlns="http://www.w3.org/2000/svg" className={className} aria-label="XXL">
        <text x="168" y="155" fontFamily="Rubik, Arial Black, sans-serif" fontWeight="900" fontSize="160" fill="#064E3B" textAnchor="middle" fontStyle="italic" letterSpacing="-4">XXL</text>
        <text x="160" y="150" fontFamily="Rubik, Arial Black, sans-serif" fontWeight="900" fontSize="160" fill="#059669" textAnchor="middle" fontStyle="italic" letterSpacing="-4">XXL</text>
      </svg>
    );
  }
  ...
  const tagline = lang === 'he' ? (
    <g className="xxl-tagline">
      ...
      <text fontFamily="Rubik, Arial, sans-serif" fontWeight="900" fontSize="28" fill="#EA580C" letterSpacing="-0.5">
        <textPath href={`#${arcId}`} startOffset="50%" textAnchor="middle">
          קונים חכם · חוסכים בענקקק
        </textPath>
      </text>
    </g>
  ) : (
  ...
  return (
    <svg
      ...
      className={combined}
      aria-label={lang === 'he' ? 'XXL — קונים חכם · חוסכים בענקקק' : 'XXL — SAVING BIG'}
    >
      ...
      <g className="xxl-shake">
        ...
        <text className="xxl-shadow" x="348" y="271" fontFamily="Rubik, Arial, sans-serif" fontWeight="900" fontSize="170" fill="#064E3B" textAnchor="middle" fontStyle="italic" letterSpacing="-4">XXL</text>
        <text className="xxl-front"  x="340" y="265" fontFamily="Rubik, Arial, sans-serif" fontWeight="900" fontSize="170" fill="#059669" textAnchor="middle" fontStyle="italic" letterSpacing="-4">XXL</text>
        {isHero && (
          <rect className="xxl-flash" x="0" y="0" width="680" height="320" fill="#FFFFFF" opacity="0" pointerEvents="none" />
        )}
        {tagline}
      </g>
    </svg>
