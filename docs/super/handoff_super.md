...
> **OUTCOME — the Aug 5 10:00 cron ran clean, the first success since Aug 2.** `status=0/SUCCESS`, not killed; `Cron finished in 10747s. Errors: none`; full coverage report emitted, **863/943 stores (91.5%), 0 errors**. That is ~3h against 13,828-19,314s for the pre-delta successful runs — a 22-46% cut, consistent with all 14 chains now on delta. Treat the delta migration as having fixed the symptom. **This is strong evidence, not proof of the cause:** a shorter run cannot trigger a late-run resource ceiling, so a run that finishes sooner would look identical whether or not memory was the trigger. **The kernel-log check was never actually run** (it needs sudo), so the OOM hypothesis remains unconfirmed — if the kill ever recurs, run `sudo journalctl -k` first rather than re-deriving from scratch.
